...
def mutate_tensor(x, indices, values):
  ...
  for indice, value in zip(indices, values):
      # print(list(zip(indices, values)))
      x[indice[0], indice[1]] = value
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return x
...
def count_tensor_elements(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  num_elements = len(x.view(-1).tolist())
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return num_elements
...
def multiples_of_ten(start, stop):
  ...
  for num in range(start, stop):
      if num % 10 == 0:
          num_list.append(num)
  x = torch.tensor(num_list, dtype=torch.float64)
...
def slice_indexing_practice(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  last_row = x[-1, :]  # one-dimensional tensor
  third_col = x[:, 2:3] # two-dimensional tensor
  first_two_rows_three_cols = x[:2, :3]
  even_rows_odd_rols = x[::2, 1::2]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  out = (
...
def reverse_rows(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  idx1 = list(range(x.shape[0]))
  idx1.sort(reverse=True)
  y = x[idx1, :] # or y = x[torch.tensor(idx1), :]
  #############################################################################
  #                            END OF YOUR CODE                               #
  #############################################################################
  return y
...
def take_one_elem_per_col(x):
  ...
  - The first element of y is the second element of the first column of x  x[1, 0]
  - The second element of y is the first element of the second column of x x[0, 1]
  - The third element of y is the fourth element of the third column of x  x[3, 2]
  """
  y = None
  #############################################################################
  #                    TODO: Implement this function                          #
...
def reshape_practice(x):
  ...
  y[0] = x[:12].view(3, 4)
  y[1] = x[12:].view(3, 4)
  y = y.permute(1, 0, 2).contiguous().view(3, 8)
...
def zero_row_min(x):
  ...
  #############################################################################
  # Replace "pass" statement with your code
  max_idxs = torch.argmin(x, dim=1) # 1D-vector
...
def batched_matrix_multiply(x, y, use_loop=True):
  ...
  if use_loop:
      ...
      for x_i, y_i in zip(x, y):
          z_list.append(torch.mm(x_i, y_i))
      z = torch.stack(z_list, 0)
  else:
      z = torch.bmm(x, y)
...
def normalize_columns(x):
  ...
  # Replace "pass" statement with your code
  #y = x.clone()
  #y_mean = torch.sum(x, 0) / x.shape[0] # shape (N,)
  #y_std = torch.sqrt(torch.sum(((x - y_mean) ** 2), 0) / (x.shape[0] - 1))
  #y = (x - y_mean) / y_std
